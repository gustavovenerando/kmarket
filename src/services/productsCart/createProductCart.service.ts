...
const createProductCartService = async ({
 quantity,
 productId,
}: IProductCartRequest, cartId: string): Promise<ProductsCart> => {
...
console.log("akiiii", productCartId);
  ...
  if(productCartId  === "") {
    ...
    await productsCartRepository.update(productCart!.id, {quantity: newQuantity});
